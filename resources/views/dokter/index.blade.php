                                      ...
                                      @can('manage-items', App\User::class)
                                            <td class="text-right">
                                                @if (auth()->user()->can('update', $dokter) || auth()->user()->can('delete', $dokter))
                                                    @can('update', $dokter)
                                                        <a type="button" href="{{route("dokter.edit",$dokter)}}" rel="tooltip" class="btn btn-success btn-icon btn-sm " data-original-title="" title="">
                                                            <i class="now-ui-icons ui-2_settings-90"></i>
                                                        </a>
                                                    @endcan
                                                    @can('delete', $dokter)
                                                        <form action="{{ route('dokter.destroy', $dokter) }}" method="post" style="display:inline-block;" class ="delete-form">
                                                            @csrf
                                                            @method('delete')
